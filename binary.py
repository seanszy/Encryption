def to_binary(long_key):
    ...
    long_key_str = str(long_key_hex) #convert to str
...
def from_binary(long_key_binary):
    ...
    count = 0 #count used because a char is represented by every 3 digit int
    for value in range(int(range_variable)): #for loop converts each int to char
        ...
        count+=3 #updates to next int representing a char
